...
export class PurchaseRequestComponent implements OnInit {
  ...
  onSubmit() {
    ...
    const selectedCategory = this.categoryList.find(
      (obj: { _id: any }) => obj._id === requestData.title
    );
    ...
    requestData['expected_delivery_date'] = new Date(
        ...
        for (let item of requestData.items) {
          ...
          if (tempobj.item) {
            ...
            tempobj.Total =
              ...
              (item.qty *
                item.rate *
                (tempobj.item.tax ? tempobj.item.tax.amount : 0)) /
                100;
    ...
    formData.append(
      ...
      formValue.purchase_request_number
    );
          ...
          if (item.files && Array.isArray(item.files)) {
            item.files.forEach((file, fileIndex) => {
              if (file && file.name) {
                ...
                console.warn(
                  `Invalid file object for item ${index}, file index ${fileIndex}`
                );
              }
            });
          } else {
  ...
  getList() {
    ...
    const vendor = this.http.get<any>(
      `${environment.api_path}${GET_VENDOR_API}`
    );
    const brand = this.http.get<any>(`${environment.api_path}${GET_BRAND_API}`);
    const category = this.http.get<any>(
      `${environment.api_path}${CATEGORY_API}`
    );
    this.httpService
      .multipleRequests([UOM, item, vendor, brand, site, category], {})
      .subscribe((res) => {
        if (res) {
  ...
  */
  ...
  onVendorSelection(event: any) {
    ...
    if (selectedVendor) {
      ...
      this.filteredItemList = this.itemList.filter((item) => {
        const categoryMatch = selectedVendor.category.some(
          (categoryItem) => categoryItem === item.category
        );
        const subCategoryMatch = selectedVendor.SubCategory.some(
          (subCategoryItem) => subCategoryItem === item.sub_category
        );
  ...
  toggleBrandSelection(brandId: string, index: number): void {
    const brandSelections = (
      this.purchaseRequestForm.get('items') as FormArray
    ).controls[index].get('brandSelections') as FormArray;
    const existingIndex = brandSelections.controls.findIndex(
      (control) => control.value === brandId
    );
  ...
  selectedTitle(event: any) {
    const title = this.categoryList.find(
      (obj: { _id: any }) => obj._id == event.value
    );

    const dynamicDataFormatted = title.name.replace(/[ ,]/g, '_');

    this.filteredItemList = this.itemList.filter(
      (item) => item.category === event.value
    );
    this.itemCategoryList = this.itemList.filter(
      (item) => item.category === event.value
    );
    this.items = [];
  ...
  selectedSite(event: any) {
    const siteName = this.siteList.find(
      (obj: { _id: any }) => obj._id == event.value
    );
    ...
    const purchase = this.http.get<any>(
      `${environment.api_path}${PURCHASE_REQUEST_API}`
    );
    ...
    this.httpService.GET(PURCHASE_REQUEST_API, filterObj).subscribe({
      next: (resp: any) => {
        ...
        resp.data.forEach((purchaseRequest: any) => {
          purchaseRequest.date = moment(purchaseRequest.updated_at).format(
            'DD-MM-YYYY'
          );
        });
  ...
  dateFilter(event: MatDatepickerInputEvent<Date>) {
    if (this.originalPurchaseList && this.originalPurchaseList.length > 0) {
      if (event.value) {
        this.purchaseList = this.originalPurchaseList.filter(
          (obj) => new Date(obj.date) == new Date(event.value)
        );
      } else {
  ...
  removeFile(index: number, file: File): void {
    ...
    const updatedFiles = filesArray.value.filter(
    ...
    if (this.originalPurchaseList && this.originalPurchaseList.length > 0) {
      if (type == 'site') {
        if (event.target.value) {
          this.purchaseList = this.originalPurchaseList.filter((obj) =>
            obj.siteData.site_name
              .toLowerCase()
...
let theList=this.getBrandsForItem(selectedItem);
  ...
  searchItem(event: any) {
    ...
    if (this.itemList && this.itemList.length > 0) {
      if (event.target.value) {
        this.filteredItemList = this.itemCategoryList.filter((obj) =>
          obj.item_name.toLowerCase().includes(searchValue)
        );
      } else {
  ...
  /*searchBrand(event: any) {
    ...
    if (this.brandList && this.brandList.length > 0) {
      if (event.target.value) {
        this.filteredBrandList = this.brandList.filter((obj) =>
          obj.brand_name.toLowerCase().includes(searchValue)
        );
      } else {
  ...
  ngOnInit(): void {
    ...
    this.httpService.GET(GET_ROLE_API_PERMISSION, {}).subscribe({
      next: (resp: any) => {
        this.viewPermission =
          resp.dashboard_permissions[0].ParentChildchecklist[18].childList[0].isSelected;
        this.addPermission =
          resp.dashboard_permissions[0].ParentChildchecklist[18].childList[1].isSelected;
        this.editPermission =
          resp.dashboard_permissions[0].ParentChildchecklist[18].childList[2].isSelected;
      },
      error: (err) => {
        console.log(err);
      },
