...
export class ListingComponent implements OnInit {
    ...
    this.userService.getUserss().subscribe((users) => {
      ...
      if (currentUser) {
        ...
        this.httpService.GET(GET_ROLE_API_PERMISSION, {}).subscribe({
          next: (resp: any) => {
            this.addPermission =
              resp.dashboard_permissions[0].ParentChildchecklist[7].childList[0].isSelected;
            this.editPermission =
              resp.dashboard_permissions[0].ParentChildchecklist[7].childList[1].isSelected;
            this.deletePermission =
              resp.dashboard_permissions[0].ParentChildchecklist[7].childList[2].isSelected;
            this.viewPermission =
              resp.dashboard_permissions[0].ParentChildchecklist[7].childList[3].isSelected;
          },
          error: (err) => {
            console.log(err);
          },
  ...
  getList() {
    this.httpService.GET(ACTIVITY_API, {}).subscribe(
      (res) => {
        if (res && res.data) {
  ...
  edit(id: any) {
    if (!this.editPermission) {
      this.toast.openSnackBar(
        'Access to Sub Activity Master editing is restricted for your account.'
      );
      return;
    }
  ...
  add() {
    if (!this.addPermission) {
      this.toast.openSnackBar(
        'Access to Sub Activity Master add is restricted for your account.'
      );
      return;
    }
    const confirmDialogPopup = this.dialog.open(AddDataComponent, {});
  ...
  delete(id: any) {
    if (!this.deletePermission) {
      this.toast.openSnackBar(
        'Access to Sub Activity Master deleting is restricted for your account.'
      );
      return;
    }
    this.httpService.DELETE(ACTIVITY_API, { _id: id }).subscribe(
      (res) => {
        if (res) {
  ...
  search(event: any) {
    if (event.target.value) {
      this.activityList = this.list.filter((obj) =>
        obj.activity_name
          .toLowerCase()
  ...
  async exportXlSX() {
    let sheetHeaders = ['Activity Name	'];
