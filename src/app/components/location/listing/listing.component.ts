...
export class ListingComponent implements OnInit {
  ...
  constructor(
    ...
    const GET_ROLE_API_PERMISSION = `/roles/role/${rolePermission}`;  
      this.httpService.GET(GET_ROLE_API_PERMISSION,{}).subscribe({
        next: (resp: any) => {
          this.addPermission=resp.dashboard_permissions[0].ParentChildchecklist[8].childList[0].isSelected;
          this.editPermission=resp.dashboard_permissions[0].ParentChildchecklist[8].childList[1].isSelected;
          this.deletePermission=resp.dashboard_permissions[0].ParentChildchecklist[8].childList[2].isSelected;
          this.viewPermission=resp.dashboard_permissions[0].ParentChildchecklist[8].childList[3].isSelected;
        },
        error: (err) => {
          console.log(err)
        }
  ...
  getList() {
    this.httpService.GET(LOCATION_API, {}).subscribe(res => {
      if (res && res.data) {
  ...
  search(event: any) {
    if (event.target.value) {
      this.structureList = this.list.filter(obj => obj.location_name.toLowerCase().includes(event.target.value.toLowerCase()))
    }
    else {
