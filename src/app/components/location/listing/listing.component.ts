...
export class ListingComponent implements OnInit {
    ...
    this.userService.getUserss().subscribe((users) => {
      ...
      if (currentUser) {
        ...
        this.httpService.GET(GET_ROLE_API_PERMISSION, {}).subscribe({
          next: (resp: any) => {
            this.addPermission =
              resp.dashboard_permissions[0].ParentChildchecklist[8].childList[0].isSelected;
            this.editPermission =
              resp.dashboard_permissions[0].ParentChildchecklist[8].childList[1].isSelected;
            this.deletePermission =
              resp.dashboard_permissions[0].ParentChildchecklist[8].childList[2].isSelected;
            this.viewPermission =
              resp.dashboard_permissions[0].ParentChildchecklist[8].childList[3].isSelected;
          },
          error: (err) => {
            console.log(err);
          },
  ...
  getList() {
    this.httpService.GET(LOCATION_API, {}).subscribe(
      (res) => {
        if (res && res.data) {
  ...
  edit(id: any) {
    if (!this.editPermission) {
      this.toast.openSnackBar(
        'Access to Location Master editing is restricted for your account.'
      );
      return;
    }
  ...
  add() {
    if (!this.addPermission) {
      this.toast.openSnackBar(
        'Access to Location Master add is restricted for your account.'
      );
      return;
    }
    const confirmDialogPopup = this.dialog.open(AddDataComponent, {});
  ...
  delete(id: any) {
    if (!this.deletePermission) {
      this.toast.openSnackBar(
        'Access to Location Master deleting is restricted for your account.'
      );
      return;
    }
    this.httpService.DELETE(LOCATION_API, { _id: id }).subscribe(
      (res) => {
        if (res) {
  ...
  search(event: any) {
    if (event.target.value) {
      this.structureList = this.list.filter((obj) =>
        obj.location_name
          .toLowerCase()
  ...
  async exportXlSX() {
    let sheetHeaders = ['Structure Name	'];

    let valueKey = ['location_name'];
