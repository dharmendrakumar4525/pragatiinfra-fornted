...
export class MaterialRecordSheetComponent implements OnInit {
  ...
  constructor(
    ...
    this.GetApprovedpurchaseOrderList({
  ...
  ngOnInit(): void {
    ...
    this.userService.getUserss().subscribe((users) => {
      ...
      if (currentUser) {
        this.DMRPermissionsView =
          this.permissions.permissions[0]?.ParentChildchecklist[15]?.childList[1];
        this.DMRPermissionsEdit =
          this.permissions.permissions[0]?.ParentChildchecklist[15]?.childList[0];
        //console.log(this.DMRPermissionsView)
        // Define your observables
        const recentActivities$ =
          this.recentActivityService.getRecentAtivities();
  ...
  GetApprovedpurchaseOrderList(filterObj: any) {
    this.httpService.GET(DMRPURCHASE_ORDER_API, filterObj).subscribe(
      (res) => {
        //console.log(res);
        if (res && res.data) {
          this.ApprovedpurchaseOrderList = res.data;
          this.OriginalApprovedpurchaseOrderList = res.data;
          this.ApprovedpurchaseOrderList.sort((a, b) =>
            a.date < b.date ? 1 : -1
          );
          this.OriginalApprovedpurchaseOrderList.sort((a, b) =>
            a.date < b.date ? 1 : -1
          );
        }
      },
  ...
  getAllLocation() {
    this.httpService.GET(SITE_API, {}).subscribe(
      (res) => {
        if (res && res.data) {
          this.site = res.data;
  ...
  getLocationById(id: string) {
    ...
    if (resultobj) return resultobj.location;
    return;
  }
  ...
  getHandelByRateApprovaId(RateApprovalById: string) {
    ...
    let resultData = this.RateApprovalList.find(
      (obj) => obj._id == RateApprovalById
    );
    //console.log(resultData)
    let resultobj =
      this.UserList.find((obj) => obj._id == resultData.handle_by) || null;
    return resultobj;
  }
  ...
  getRateApprovalById(id: string) {
    //console.log(this.RateApprovalList)
    let resultobj =
      this.RateApprovalList.find((item) => item._id == id) || null;
    //console.log(resultobj);
    if (resultobj != null)
  ...
  search(event: any, type?: any) {
    console.log(type);
    if (
      this.OriginalApprovedpurchaseOrderList &&
      this.OriginalApprovedpurchaseOrderList.length > 0
    ) {
      if (type == 'location') {
        if (event.target.value) {
          this.ApprovedpurchaseOrderList =
            this.OriginalApprovedpurchaseOrderList.filter((obj) =>
              this.getLocationById(obj.site)
                .toLowerCase()
        ...
        if (event.target.value) {
          this.ApprovedpurchaseOrderList =
            this.OriginalApprovedpurchaseOrderList.filter((obj) =>
              this.getRateApprovalById(obj.rate_approval_id)
                .purchase_request_number.toLowerCase()
                .includes(event.target.value.toLowerCase())
            );
        } else {
          this.ApprovedpurchaseOrderList =
  ...
  showSnackbar() {
    if (!this.DMRPermissionsEdit.isSelected) {
      this.toast.openSnackBar(
        'Access to DMR editing is restricted for your account.'
      );
    }
  }
