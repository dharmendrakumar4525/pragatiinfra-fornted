...
export class MasteruserComponent implements OnInit {
  ...
  checked = false;
  displayedColumns = ['SelectAll', 'No', 'taskName', 'Action'];
  dataSource = null;
  users: any;
  usersLen: any;
  length = 50;
  ...
  ngOnInit(): void {
    ...
    this.userService.getUserss().subscribe((users) => {
      ...
      if (currentUser) {
        this.tasksPermissionsAdd =
          this.permissions.permissions[0]?.ParentChildchecklist[6]?.childList[0];
        this.tasksPermissionsEdit =
          this.permissions.permissions[0]?.ParentChildchecklist[6]?.childList[1];
        this.tasksPermissionsDelete =
          this.permissions.permissions[0]?.ParentChildchecklist[6]?.childList[2];
    ...
    this.taskService.deleteTask(id).subscribe({
      ...
      error: (err) => {
        this.toast.openSnackBar(
          'Something went wrong. Unable to delete Activity'
        );
      },
    });
  }
