...
export class SubActivitiesComponent implements OnInit {
  ...
  checked = false;
  displayedColumns = ['SelectAll', 'No', 'subTaskName', 'Action'];
  dataSource = null;
  users: any;
  usersLen: any;
  length = 50;
  ...
  ngOnInit(): void {
    ...
    this.subActPermissionsAdd =
      this.permissions.permissions[0]?.ParentChildchecklist[7]?.childList[0];
    this.subActPermissionsEdit =
      this.permissions.permissions[0]?.ParentChildchecklist[7]?.childList[1];
    this.subActPermissionsDelete =
      this.permissions.permissions[0]?.ParentChildchecklist[7]?.childList[2];
    ...
    this.taskService.deleteSubActivity(id).subscribe({
      ...
      error: (err) => {
        this.toast.openSnackBar(
          'Something went wrong. Unable to delete sub activity'
        );
      },
    });
  }
