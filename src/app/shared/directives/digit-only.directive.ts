...
export class DigitOnlyDirective implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges | any): void {
    if (changes.pattern) {
      this.regex = this.pattern ? RegExp(this.pattern) : RegExp('');
    }
  ...
  onPaste(event: ClipboardEvent) {
    const pastedInput: any = event.clipboardData?.getData('text/plain');
  ...
  private pasteData(pastedContent: string): void {
    ...
    if (!pasted) {
      ...
      this.inputElement.setRangeText(sanitizedContent);
    }
    this.updateDecimalPoint();
  }
  ...
  private getSelection(): string {
    return this.inputElement.value.substring(
      this.inputElement.selectionStart ? this.inputElement.selectionStart : 0,
      this.inputElement.selectionEnd ? this.inputElement.selectionEnd : 0
    );
  }
